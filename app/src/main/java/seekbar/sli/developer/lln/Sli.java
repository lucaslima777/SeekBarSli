...
public class Sli extends FrameLayout {
    ...
    public Sli(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Sli(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void setCurrentValueNoUpdate(float value) {
        this.currentValue = value;
        listener.valueChanged(Sli.this, currentValue);
        updateValues();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.save();
        {
            ...
            if (isRegions()) {
                if (steps.isEmpty()) {
                    settings.paintIndicator.setColor(settings.regionColorLeft);// color ball ring
            ...
            //indicator
            {
                ...
                settings.paintIndicator.setColor(Color.parseColor("#003BCF")); //draw color circle
    ...
    public interface Listener {
        void valueChanged(Sli sli, float currentValue);
    }
    ...
    public static class Settings {
        ...
        private void init(Context context, AttributeSet attrs) {
            if (attrs != null) {
                final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Sli);
                setColorBackground(a.getColor(R.styleable.Sli_slidr_backgroundColor, colorBackground));

                this.step_colorizeAfterLast = a.getBoolean(R.styleable.Sli_slidr_step_colorizeAfterLast, step_colorizeAfterLast);
                this.step_drawLines = a.getBoolean(R.styleable.Sli_slidr_step_drawLine, step_drawLines);
                this.step_colorizeOnlyBeforeIndicator = a.getBoolean(R.styleable.Sli_slidr_step_colorizeOnlyBeforeIndicator, step_colorizeOnlyBeforeIndicator);

                this.drawTextOnTop = a.getBoolean(R.styleable.Sli_slidr_textTop_visible, drawTextOnTop);
                setTextTopSize(a.getDimensionPixelSize(R.styleable.Sli_slidr_textTop_size, (int) dpToPx(textTopSize)));
                this.drawTextOnBottom = a.getBoolean(R.styleable.Sli_slidr_textBottom_visible, drawTextOnBottom);
                setTextBottomSize(a.getDimensionPixelSize(R.styleable.Sli_slidr_textBottom_size, (int) dpToPx(textBottomSize)));

                this.barHeight = a.getDimensionPixelOffset(R.styleable.Sli_slidr_barHeight, (int) dpToPx(barHeight));
                this.drawBubble = a.getBoolean(R.styleable.Sli_slidr_draw_bubble, drawBubble);
                this.modeRegion = a.getBoolean(R.styleable.Sli_slidr_regions, modeRegion);

                this.regionColorLeft = a.getColor(R.styleable.Sli_slidr_region_leftColor, regionColorLeft);
                this.regionColorRight = a.getColor(R.styleable.Sli_slidr_region_rightColor, regionColorRight);

                this.indicatorInside = a.getBoolean(R.styleable.Sli_slidr_indicator_inside, indicatorInside);
                this.regions_textFollowRegionColor = a.getBoolean(R.styleable.Sli_slidr_regions_textFollowRegionColor, regions_textFollowRegionColor);
                this.regions_centerText = a.getBoolean(R.styleable.Sli_slidr_regions_centerText, regions_centerText);

                this.editOnBubbleClick = a.getBoolean(R.styleable.Sli_slidr_edditable, editOnBubbleClick);
        ...
        private float dpToPx(int size) {
            return size * sli.getResources().getDisplayMetrics().density;
        }
    ...
    public class EurosTextFormatter implements TextFormatter {
        ...
        public String format(float value) {
            return String.format("%d XP", (int) value);
        }
    }
}
